...
DATA_FILE = SOURCE_DIR / "swatches.json"
VERSION_FILE = SOURCE_DIR / "swatches_version.json"
LAST_CHECKED_FILE = SOURCE_DIR / "last_checked.json"
...
def download_filament_info():
    ...
    try:
        if LAST_CHECKED_FILE.exists():
            ...
            with open(LAST_CHECKED_FILE, "w", encoding="utf-8") as f:
                ...
                json.dump({"db_last_modified": time.time()}, f, indent=2)
    ...
    with open(LAST_CHECKED_FILE, "w", encoding="utf-8") as f:
        json.dump({"db_last_modified": api_version.get("db_last_modified")}, f, indent=2)
    
    print(f"Download complete! Downloaded {len(results)} swatches")
